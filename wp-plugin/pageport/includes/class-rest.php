<?php
/**
 * REST routes for PagePort Smart Share (v2.2).
 *
 * Authenticated routes (cookie + X-WP-Nonce):
 *   POST   /pageport/v1/sessions
 *   GET    /pageport/v1/sessions
 *   DELETE /pageport/v1/sessions/{id}
 *
 * Public routes (no auth, served as raw bytes):
 *   GET /pageport/v1/share/{id}/index.html
 *   GET /pageport/v1/share/{id}/style.css
 *   GET /pageport/v1/share/{id}/script.js
 *   GET /pageport/v1/share/{id}/preview.png
 */
...
final class PagePort_REST {
    ...
    const SLUGS = [
        'index.html'  => PagePort_AssetType::HTML,
        'style.css'   => PagePort_AssetType::CSS,
        'script.js'   => PagePort_AssetType::JS,
        'preview.png' => PagePort_AssetType::IMAGE,
    ];
    ...
    public static function create_session( WP_REST_Request $req ) {
        ...
        foreach ( [ 'html', 'css', 'js', 'image' ] as $k ) {
            if ( empty( $files[ $k ]['tmp_name'] ) || ! empty( $files[ $k ]['error'] ) ) {
                return new WP_Error(
                    PagePort_ErrorCode::E_SHARE_BAD_INPUT,
                    "missing field: $k",
                    [ 'status' => 400 ]
                );
            }
    ...
    public static function read_asset( WP_REST_Request $req ) {
        ...
        if ( ! isset( self::SLUGS[ $slug ] ) ) {
            return new WP_Error( PagePort_ErrorCode::E_SHARE_NOT_FOUND, 'unknown asset', [ 'status' => 404 ] );
        }
    ...
    private static function content_type_for( $kind, $stored_mime ) {
        switch ( $kind ) {
            case PagePort_AssetType::HTML:  return 'text/html; charset=utf-8';
            case PagePort_AssetType::CSS:   return 'text/css; charset=utf-8';
            case PagePort_AssetType::JS:    return 'application/javascript; charset=utf-8';
            case PagePort_AssetType::IMAGE:
    ...
    private static function guess_mime( $kind ) {
        switch ( $kind ) {
            ...
            case PagePort_AssetType::JS:    return 'application/javascript';
    ...
    private static function strip_exif( $tmp_path, $mime ) {
        ...
        $dest     = wp_tempnam( 'pageport-img-' . $ext );
