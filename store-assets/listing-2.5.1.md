# Inspect Page — Chrome Web Store listing copy (v2.5.1)

_Submit this version when uploading `public/inspect-page.zip` (extension manifest 2.5.1) to the Chrome Web Store. Diff vs `listing-2.5.0.md`: short-description tweak + a new "On-page picker chip" bullet under the inspector section + Privacy/Permissions unchanged._
...
### Built-in inspector (refined in 2.5.1)
...
v2.5.1 — Picker chip is now interactive: ✓ Select / ⧉ Copy selector / ✕ Cancel buttons sit right next to the highlighted element so you no longer have to right-click. Hovering the chip freezes the highlight while you aim. Fixed an Inspector styling regression where the Copy / Anchor / Show Code buttons rendered as stacked underlined links instead of a horizontal toolbar.