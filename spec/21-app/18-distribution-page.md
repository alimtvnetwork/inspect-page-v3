...
A single page in the Lovable host project that distributes `public/inspect-page.zip`.
...
`{version}` and `{sizeKb}` resolved at build time from `public/inspect-page.zip`.

## Download button behavior

- The Lovable preview environment requires authenticated fetches for static files. Use **fetch + blob**:
  1. `res = await fetch('/inspect-page.zip')`
  2. If `!res.ok` → show inline error `Download failed: {status}`.
  3. `blob = await res.blob()`.
  4. Create `<a href={URL.createObjectURL(blob)} download="inspect-page.zip">` and `.click()`.
  5. Revoke object URL.
- Disable button while in flight; show spinner.
- After success, show toast `Downloaded inspect-page.zip`.
...
## SEO
- `<title>`: `Inspect Page — Chrome extension` (≤ 60 chars).
