#!/usr/bin/env python3
"""SQLI-ORDER-001 — orderBy/groupBy identifiers must be literal or allow-listed (TS)."""
    ...
    for m in ORDER_GROUP_CALL_RE.finditer(text):
        ...
        findings.append(Finding(
            ...
            message=(
                ...
                f"Validate against an explicit allow-list (e.g. a const "
                f"`ALLOWED_COLUMNS`) before reaching the builder."
            ),
...
def main() -> int:
    args = build_parser("SQLI-ORDER-001 orderBy/groupBy idents (TS/JS)").parse_args()
    globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(
        tool_name="coding-guidelines-sqli-order-group-by-ts",
