#!/usr/bin/env python3
"""CODE-RED-004 — Function length. TypeScript / JavaScript."""
...
PATTERNS = [
    ...
    re.compile(r"^\s*(?:public|private|protected|static)?\s*(?:async\s+)?([A-Za-z_$][A-Za-z0-9_$]*)\s*\([^)]*\)[^{]*\{"),
]
...
def count_effective(lines: list[str]) -> int:
    ...
    return _count_effective_shared(lines, "typescript")


def match_function(line: str) -> str | None:
    for pat in PATTERNS:
        m = pat.match(line)
