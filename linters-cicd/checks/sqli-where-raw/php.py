#!/usr/bin/env python3
"""SQLI-RAW-002 — whereRaw() must use strict ? / :param placeholders (PHP).

Flags two patterns:
  error   — whereRaw('... $var ...')  (interpolation / concatenation)
        ...
        if not has_placeholders(arg) and not second_arg_present(text, span[1]):
            findings.append(Finding(
                ...
                message=(
                    ...
                    "params array — confirm the clause contains zero "
                    "dynamic values, otherwise switch to where()."
                ),
                file_path=relpath(path, root),
                start_line=_line_of(text, m.start()),
            ))
...
def main() -> int:
    args = build_parser("SQLI-RAW-002 whereRaw strict params (PHP)").parse_args()
    globs = parse_exclude_paths(args.exclude_paths)
    run = SarifRun(
        tool_name="coding-guidelines-sqli-where-raw-php",
