...
import { ShowCodeDrawer } from "./ShowCodeDrawer";
...
export function InspectInspector({ snapshot }: InspectInspectorProps): JSX.Element {
  ...
  const codeSample = codeIdx !== null ? samples[codeIdx] ?? null : null;
  ...
  return (
    <section className="lpe-inspector" aria-label={COPY.inspectInspectorTitle}>
      ...
      {codeSample && (
        <ShowCodeDrawer sample={codeSample} onClose={() => setCodeIdx(null)} />
      )}
    </section>
  );
}
