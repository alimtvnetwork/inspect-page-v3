...
Inspect Page exports the web page in the active tab — its HTML, CSS,
...
|---|---|
| `activeTab` | Required so Inspect Page can read the DOM and styles of the tab the user explicitly invoked it on. Without `activeTab` the extension cannot serialize the page the user wants to export. |
...
| Host permission `<all_urls>` | The user may invoke Inspect Page on any HTTP(S) page they visit. The extension does not run in the background; the content script only loads when the user triggers an export on a given tab. |
...
- [x] We do **not** collect authentication information beyond what the
      ...
      page. No passwords are stored by Inspect Page.
